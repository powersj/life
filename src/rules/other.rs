...
pub fn update_grid(grid: &[Vec<i32>]) -> Vec<Vec<i32>> {
    ...
    for x in 0..GRID_SIZE {
        for y in 0..GRID_SIZE {
            ...
            // Any live cell with fewer than three live neighbors lives, as if by under-population.
            // Any live cell with three or more live neighbors dies, as if by over-population.
            // Any dead cell with exactly two live neighbors becomes a live cell, as if by reproduction.
            if grid[x][y] == 1 && count >= 3{
                new_grid[x][y] = 0;
            } else if grid[x][y] == 0 && count == 2 {
                new_grid[x][y] = 1;
            } else {
